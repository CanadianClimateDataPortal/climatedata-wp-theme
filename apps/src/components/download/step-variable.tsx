...
const StepVariable = React.forwardRef<StepComponentRef>((_, ref) => {
	...
	useEffect(() => {
		...
		if (!hasCurrent) {
			...
			// Track that auto-selection has occurred
			setAutoSelectedDone(true);
		}
	}, [dataset, filteredList, climateVariable, selectClimateVariable]);

	// Handle auto-selected variable URL update
	useEffect(() => {
		if (autoSelectedDone && climateVariable && dataset) {
			// Sync the URL with the auto-selected variable
	...
	}, [autoSelectedDone, climateVariable, dataset]);

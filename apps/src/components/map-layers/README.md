...
- `selectable-rectangle-grid-layer.tsx` → `SelectableRectangleGridLayer` — draw/resize a rectangle selection with cell-count limit.
- `map-legend-open-control.tsx` → `MapLegendOpenControl` — collapsible legend toggle button and panel shell.
- `interactive-regions-layer.tsx` → `InteractiveRegionsLayer` — colours the map by grid/watershed/health/census region.

...
/*
    $('.accordion-content').scroll(function() {
      
      if ($(this).find('.fade').length) {
        
        var this_height = $(this).find('.accordion-content-inner').outerHeight()
        
        if ($(this).scrollTop() >= this_height) {
          $(this).find('.fade').fadeOut(100)
        } else {
          $(this).find('.fade').fadeIn(100)
        }
    ...
    function validate_steps() {
      ...
      $('#analyze-steps .validate-input').each(function(i) {
        ...
        if (is_valid == true) {
          ...
          // set text & show step
          $('#analyze-breadcrumb [data-step="' + current_step + '"]').addClass('on').find('.value').text(breadcrumb_val)
...
/*
      if (
        zoom_on == false && 
        $('.accordion-head[data-step="1"]').attr('data-valid') == 'true'
      ) {
...
/*
      if ($('#analyze-model-all').closest('.select-all').hasClass('checked')) {
        form_inputs['models'] = '24MODELS'
      }
*/

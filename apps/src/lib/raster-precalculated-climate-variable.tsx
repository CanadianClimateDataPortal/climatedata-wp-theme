...
/**
 * Return ids of pre-calculated CanDCS variables available for a specific frequency type.
 *
 * The `preCalculatedCanDCSConfig` configuration is used to determine variables available
 * for the frequency, and their pre-calculated id.
 *
 * @param frequency Frequency type for which we want available variables.
 */
function getCanDCSVariableIds(frequency: FrequencyType): string[] {
	...
	async getDownloadUrl(): Promise<string | null> {
		...
		if (payload.var !== 'all') {
			...
			if (payload.month) {
				...
				if (frequencyType) {
					availableVars = getCanDCSVariableIds(frequencyType);
				}
			}

...
- `createFetchTargetToRasterWithEncodedUrl` — builds the salted, encoded screenshot-service URL; `resolveRasterFetchTarget`'s fallback when `window.MAP_RASTER_PROXYPHP_DATA_URL` is absent, and always the target `createRasterDebugger`'s `createFetchFor` retargets at a different screenshot-service deployment.
- `resolveRasterFetchTarget` — picks the Sender's POST target: `mapUrl.href` when `window.MAP_RASTER_PROXYPHP_DATA_URL` holds a URL, otherwise `createFetchTargetToRasterWithEncodedUrl`'s encoded URL. That global is optional and often repeats `window.DATA_URL`, so its presence decides the branch and an empty string counts as absent.

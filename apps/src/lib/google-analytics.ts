 ...
 *
 * If the variable is not a trackable variable, or the format is unsupported,
 * an empty event is triggered (this is required because of how the Google Tag
...
export function trackGraphExport(
	...
	if (eventVariableName) {
		switch (format) {
			case 'csv':
				eventName = `Variable_Download-Data_${eventVariableName}`;
				break;
			case 'pdf':
			case 'png':
			case 'print':
				eventName = `Variable_Download-Image_${eventVariableName}`;
				break;
		}
	}
 ...
 *
 * If the variable is not a trackable variable, an empty event is triggered
 * (this is required because of how the Google Tag Manager was configured).
 *
...
export function trackFinchDownload(
	...
	const eventName = variableName ? `Analyze_BCCAQv2_${variableName}` : '';
...
export function trackPrecalculatedDownload(
	...
	const eventName = variableName ?
		`Download_Variable-Data_BCCAQv2_${variableName}_Frequency_Location_Format`:
		'';
	let dataLocation = '';
...
export function trackStationDataDownload(
	...
	const isSupportedVariable = [
		'station_data', 'msc_climate_normals'
	...
	const variableName = getPrecalculatedVariableName(climateVariable);
	const eventName = isSupportedVariable && variableName ?
		`Download_${variableName}` :
		'';

...
[`setup.ts`](./setup.ts) is wired in through [`vite.config.ts`](../../vite.config.ts).

## jsdom platform shims

[`setup.ts`](./setup.ts) fills in two browser APIs the image-rastering tests exercise.
...
The settle loop in [`lib/map/image-rastering/wait-for-maps-settled.ts`](../lib/map/image-rastering/wait-for-maps-settled.ts) is timed against real frame scheduling, and a hand-rolled replacement would change the very timing the settle tests exist to pin.
...
- [`setup.ts`](./setup.ts) — added the two guarded shims above.

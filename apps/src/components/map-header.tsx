 ...
 */
const MapHeader: React.FC<MapInfoProps> = ({  data, mapRef }): React.ReactElement => {
  ...
  return (
    <aside className="map-header">
      <div ref={ref} className="absolute top-0 left-0 z-[9999] overflow-y-auto w-full">
        <div className="flex items-center gap-x-2 bg-white p-4">
          <Breadcrumbs title={data.title} onClick={toggleVariableDetailsPanel} />
          <ModalToggleButtons onToggleShare={toggleShareInfo} onToggleDownload={toggleDownloadInfo} />
        </div>

        <ShareMapModal isOpen={shareInfo} onClose={toggleShareInfo} />
        <DownloadMapModal isOpen={downloadInfo} onClose={toggleDownloadInfo} title={data.title} mapRef={mapRef} />
      </div>
    </aside>
  );
};
 ...
 */
const Breadcrumbs: React.FC<{ title: string; onClick: () => void }> = ({ title, onClick }) => {
  const { __ } = useI18n();
...
const ModalToggleButtons: React.FC<{
  ...
  return (
    <div className="flex gap-2 ml-auto">
      <Button variant="outline" className="rounded-full" onClick={onToggleShare}>
        {__('Share')} <span className="sr-only">{__('the current visible map')}</span>
        <Share2 />
      </Button>

      <Button variant="destructive" className="rounded-full" onClick={onToggleDownload}>
        {__('Download')} <span className="sr-only">{__('image from your viewport')}</span>
        <Download />
      </Button>
    </div>
  )

...
const ShareMapModal: React.FC<{
  ...
  const [copyLinkVariant, setCopyLinkVariant] = useState<'destructive' | 'secondary'>('destructive');
    ...
    <Modal isOpen={isOpen} onClose={onClose}>
      <div>
        ...
        <p className="text-gray-400 my-2 text-sm">
          {__('You can copy-paste this link anywhere you want; it will keep all your data options and your map position.')}
        </p>
        <div className="flex gap-4">
          <Input type="text" placeholder="crim.ca/map/?coords=62.5102..." />
          <Button
            className="rounded-full uppercase"
            variant={copyLinkVariant}
            onClick={copyLinkHandler}
          >
            {copyLinkVariant === 'secondary' ? __('Copied') : __('Copy Link')}
          </Button>
        </div>
      </div>
      <div>
        <p className="font-bold">{__('Or click one of these social links:')}</p>
        <p className="text-gray-400 my-2 text-sm">

...
- `ready-to-raster-here` on `#map-root` — the readiness signal the screenshot service polls for.
...
- `createSelectedLocationSnapshot` — builds the sender's POST payload from the open popup and marker.
- `createFetchRequestInitToRaster` — wraps a payload as the POST `fetch` init.
- `createFetchTargetToRasterWithEncodedUrl` — builds the salted, encoded screenshot-service URL.
- `signalRasterReady` — adds `ready-to-raster-here` to `#map-root`.

...
New theme and PHP code belongs in `framework/`, the parent theme, which persists.
...
`fw-child/languages/react-apps/` is the one part of this theme still in active use, and it stays here.
...
`fw-child/apps/app-map.php` and `fw-child/apps/app-download.php` both call it and hand the result to the React apps, which consume it through WordPress i18n.
...
Adding or changing a translatable string in `apps/` therefore means editing `fw-child/languages/react-apps/fr_CA.po` and recompiling:

```
./dev.sh i18n-compile-react-apps
```

That runs `wp i18n make-mo languages/react-apps/fr_CA.po` inside the container and rewrites `fr_CA.mo` beside it.

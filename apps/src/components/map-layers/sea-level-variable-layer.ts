...
/**
 * Sea Level Variable Layer Component
 * 
 * Specialized version of the variable layer for sea level data
 * that ensures it's rendered in the 'raster' pane below the landmass.
 *
 * @returns {null}
 */
...
interface SeaLevelVariableLayerProps {
	layerValue: string;
}

export default function SeaLevelVariableLayer({ layerValue }: SeaLevelVariableLayerProps): null {
	const map = useMap();
	const {
		opacity: { mapData },

...
export function AppSidebar() {
  ...
  const { setExtendInfo } = useMapContext();
	...
	const colorPalettes = [
		{ name: "Warm Palette", colors: ["#FF5733", "#FF8D1A", "#FFC300", "#FFDA6C", "#FFD1A3"] },
		{ name: "Cool Palette", colors: ["#4DA8DA", "#34A7C1", "#378AD5", "#2167A9", "#174A7B"] },
		{ name: "Neutral Palette", colors: ["#D9D9D9", "#BFBFBF", "#A6A6A6", "#8C8C8C", "#737373"] },
	];

	const handlePaletteChange = (palette: { name: string; colors: string[] }) => {
	...
	return (
		<Sidebar>
						...
						<TabsContent value="settings">
							<SidebarGroupContent>
								<SidebarMenu>
									...
									<SidebarMenuItem>
										<ColorSelect options={colorPalettes} onChange={handlePaletteChange} />
									</SidebarMenuItem>
			...
			<DatasetsPanel selected={selectedDataset} onSelect={setSelectedDataset} />
			<VariablesPanel selected={selectedVariable} onSelect={setSelectedVariable} />

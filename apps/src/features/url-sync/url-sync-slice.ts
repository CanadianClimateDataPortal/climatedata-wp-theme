...
/**
 * URL query string the app would currently write, derived from Redux
 * state — NOT from `window.location.search`.
 *
 * url-sync writes the URL with a debounced `history.replaceState` that
 * triggers no re-render, so a value read from the URL at render time lags
 * the live state (and is empty before the first write). Reading state keeps
 * this current. Memoized on the driving slices, so the language switcher
 * re-renders and rebuilds its `href` when they change.
 *

...
function child_global_vars() {
  ...
  if ( isset ( $query_string['var'] ) ) {
    ...
    $vars['current_data'] = array (
      'type' => 'variable',
      'var_name' => $query_string['var']
    );
    
    // get the matching post ID
    
    $get_post_by_slug = get_posts ( array (
      'post_type' => 'variable',
      'name' => $query_string['var'],
      'posts_per_page' => 1
    ) );
...
/*
      ...
      $terms = array ( 'City', 'Suburban Community', 'Town', 'Village' );
      ...
      foreach ( $terms as $term ) {
        ...
        if ( $continue == true ) {
          ...
          while ( $row = mysqli_fetch_assoc ( $main_query ) ) {
            ...
            echo $row['geo_name'] . ': ' . $row['generic_term'] . '<br>';
            
            if ( $row['generic_term'] == $term ) {
              $selected_place = $row;
              
              echo 'found ' . $term . ': ' . $row['geo_name'] . '<br>';
...
if ( function_exists('acf_add_options_page') ) {
  ...
  acf_add_options_page ( array (
    'page_title'    => 'Variable Archive',
    'menu_title'    => 'Archive',
    'menu_slug'     => 'options_var',
    'capability'    => 'edit_posts',
    'parent_slug'   => 'edit.php?post_type=variable',
    'position'      => false,
...
function set_case_study_parent_id() {
  ...
  if ( !empty ( $sector_query ) ) {
    ...
    foreach ( $sector_query as $sector_page ) {
      ...
      if ( count ( $ancestors ) < 3 ) echo ' disabled';
      
      if ( $current_parent == $sector_page->ID ) echo ' selected';

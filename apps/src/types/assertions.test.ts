...
import { ForecastDisplays, ForecastTypes } from '@/types/climate-variable-interface';
import { assertIsForecastDisplay, assertIsForecastType, AssertionError } from '@/types/assertions';

describe('assertIsForecastType', () => {
	test.each(
		Object.values(ForecastTypes)
	)('does not throw for valid type (%s)', (forecastType) => {
		const testCallback = () => {
...
describe('assertIsForecastDisplay', () => {
	test.each(
		Object.values(ForecastDisplays)
	)('does not throw for valid type (%s)', (forecastDisplay) => {
		const testCallback = () => {

...
export default function VariableLayer(): null {
  const map = useMap();
  const { frequency, dataset, decade, pane, opacity: { mapData } } = useAppSelector(state => state.map);
  ...
  useEffect(() => {
    ...
    const newLayer = L.tileLayer.wms(GEOSERVER_BASE_URL + '/geoserver/ows?', params);
    newLayer.addTo(map);
